...
export default function ButtonChoiceColor({ setNewTask, CATEGORY }) {
  ...
  const getChangeColor = (event) => {
    ...
    setNewTask((prev) => {
      const updated = { ...prev };
      updated.category = CATEGORY[event.currentTarget.dataset.id].category;
      return updated;
    });
        ...
        <div className={cn(styles.svgWrap)}>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              fill={CATEGORY[selectCategory].color}
          ...
          <p>{CATEGORY[selectCategory].category}</p>
        </div>
      </button>
      <ModalPalette onChangeColor={getChangeColor} isOpen={isClicked} categoryValue={CATEGORY} />
    </div>
  );
}
